    ...
    <div className='h-100 fixed-l flex flex-column justify-between' style={{ width: 'inherit' }}>
      <div className='flex flex-column'>
        <div className='pointer navy pv3 pv4-l' onClick={onToggle}>
          <img className='center' style={{ height: 70, display: open ? 'block' : 'none' }} src={ipfsLogoText} alt='OTFS' title='Toggle navbar' />
          <img className='center' style={{ height: 70, display: open ? 'none' : 'block' }} src={ipfsLogo} alt='OTFS' title='Toggle navbar' />
        </div>
        <nav className='db overflow-x-scroll overflow-x-hidden-l nowrap tc' role='menubar'>
          ...
          <NavLink to='/peers' icon={StrokeIpld} open={open}>{t('peers:title')}</NavLink>
